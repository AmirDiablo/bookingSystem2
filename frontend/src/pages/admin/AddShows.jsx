...
const AddShows = () => {
    ...
    const fetchNowPlayingMovies = async ()=> {
        try {
            const response = await fetch("http://localhost:3000/api/show/now-playing", {
                headers: {
            ...
            if(json.success) {
                setNowPlayingMovies(json.movies.results)
                console.log(json)
            }
    ...
    return nowPlayingmovies.length > 0 ? ( 
        <>
            ...
            {Object.keys(dateTimeSelection).length > 0 && (
                <div className="mt-6">
                    <h2 className="nb-2">Selected Date-Time</h2>
                                ...
                                <div className="flex flex-wrap gap-2 mt-1 text-sm">
                                    {times.map((time)=> (
                                        <div key={time} className="border border-primary px-2 py-1 flex items-center rounded">
                                            <span>{time}</span>
                                            <AiFillDelete onClick={()=> handleRemoveTime(date, time)} width={15} className="ml-2 text-red-500 hover:text-red-700 cursor-pointer" />
                                        </div>
                                    ))}
                                </div>
                            </li>
